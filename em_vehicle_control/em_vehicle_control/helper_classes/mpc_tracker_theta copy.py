...
class MPCTracker:
    ...
    def __init__(self) -> None:
        ...
        self.q_theta = 5.0  # orientation error weight
        ...
        if self.write:
            with open("src/mpc_data.txt", "w") as file:
                file.write(
                    f"Total error, OE, current theta, des OE, optimised OE \n"
                )
    ...
    def calculate_theta(
        ...
        # print(direction, flush=True)
        if np.any(np.array(direction) == 1) and np.any(np.array(direction) == -1):
    ...
    def get_reference_path(
        ...
        print(remaining_spatial_path[:3], flush = True)
