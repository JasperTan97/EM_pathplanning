...
def reeds_shepp_path_planning(sx, sy, syaw, gx, gy, gyaw, maxc, step_size=0.2):
    ...
    return b_path.x, b_path.y, b_path.yaw, b_path.directions, b_path.ctypes, b_path.lengths
...
def main():
    ...
    end_x = 3.9336363636363636   # [m]
    end_y = 2.1574999999999998  # [m]
    ...
    xs, ys, yaws, directions, modes, lengths = reeds_shepp_path_planning(start_x, start_y,
                                                             start_yaw, end_x,
                                                             end_y, end_yaw,
                                                             curvature,
                                                             step_size)
    ...
    # for x, y, direction in zip(xs, ys, directions):
    #     print(f"PathPointDatum({x:.4f}, {y:.4f}, 0, {direction}),")
    ...
    if show_animation:  # pragma: no cover
        ...
        # plotting
        robot_x, robot_y, robot_yaw = (3.872, 1.960, -2.04)
        circle1 = plt.Circle((robot_x, robot_y), 0.2, color='g')
        circle2 = plt.Circle((robot_x, robot_y), 0.02, color='r')
        ax.add_patch(circle1)
        ax.add_patch(circle2)
        plot_arrow(robot_x, robot_y, robot_yaw)
